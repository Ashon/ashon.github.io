---
title: "About"
url: "/about/"
hidemeta: true
disableShare: true
ShowReadingTime: false
ShowBreadCrumbs: false
ShowPostNavLinks: false
ShowWordCount: false
ShowRssButtonInSectionTermList: false
ShowToc: false
comments: false
---

컴퓨터와 프로그램 개발에 재미를 느끼고 공부하는 개발자 입니다. 리눅스와 파이썬을 좋아합니다. 웹 서비스 개발부터 비즈니스들을 안전하게 모니터링하고 자동화하는 기술, 그리고 클라우드 기술에 많은 흥미를 가지고 있습니다.

새로운 기술들을 빠르게 습득하고, 꼼꼼한 성격으로 심도있게 공부하는 개발자 입니다. 웹 프론트/백엔드 개발자를 시작으로 DevOps를 지나, SRE, 클라우드 비즈니스의 원리를 공부하고 서비스로 구현해 내는 일을 하고 있습니다.

오픈스택 기반의 프라이빗 클라우드 서비스 개발 운영 경험과 AWS GCP 등 퍼블릭 클라우드 기반 플랫폼 개발 경험을 가지고, 다양한 사용 계층 기술 계층에서 플랫폼 서비스를 설계하고 개발하는 경험과 역량을 갖추고 있습니다.

### Interests

- `Clean Architecture`, `Cloud`, `DevOps`, `SRE`

### Careers

- 2026.02 ~ Present - Rebellions.ai \| Cloud Platform
- 2022.12 ~ 2026.02 - 당근마켓 \| SRE
- 2018.02 ~ 2022.12 - Kakao \| Cloud Computing
- 2017.02 ~ 2017.12 - 더핑크퐁컴퍼니 \| Development Team
- 2014.08 ~ 2017.02 - CDNetworks \| Web Portal
- 2012.08 ~ 2013.12 - 네오펙트 \| Planning

### Presentations

- [당근 SRE 밋업 4회 - 멋진 운영 경험을 위하여: Prometheus 레코딩 룰로 SLI 정하기](https://youtu.be/f76i29l4aGs)
- [NIPA 제28회 SW Quality Insight Conference - Krane의 CI/CD 파이프라인](https://youtu.be/gg9Oieuv2ow?t=4501)
- [Ifkakao 2021 - 자동화된 클라우드를 위하여 v2.0](https://elseif.kakao.com/2021/session/36)
- [Ifkakao 2019 - 자동화된 클라우드를 위하여](https://elseif.kakao.com/2019/program?sessionId=a005c64e-6afa-4810-92ca-43a014aa8aac)
