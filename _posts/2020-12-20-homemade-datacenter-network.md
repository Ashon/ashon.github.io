...
- **Leaf**: 3-Tier 구성에서의 Aggregation, Access 레이어의 역할을 대신한다.
...
Docker에서는 container network 프로비저닝을 위해 `brige`, `host`, `none` 3가지 모드를
...
결과부터 말하자면 Network topology 상의 loop를 제거하기 위해서인데, 자세히는 모르지만
L2, L3 스펙 차이에서 온다고 한다.

L3(IP) 패킷에는 TTL이라고 해서 datagram spec에 패킷이 홉을 뛸 때마다 TTL을 줄여나가서
결국 0 미만이 되면 다음노드로 전달하지 않고 버리도록 되어 있으므로 패킷에 대한 라이프사이클을
컨트롤 할 수 있다.

하지만 L2에서는 TTL의 개념이 없기 때문에, ARP 요청의 해결될 때 까지 자신이 속한 네트워크로
브로드캐스트 패킷을 보내게 된다. 이 브로드캐스트 패킷은 L2 interface인 Bridge interface를
타고 다른 네트워크 노드로 전염될 수 있으며, 해결되지 않은 ARP요청이 네트워크 루프를 돌며 계속
쌓이게 되고, 끝내 과부하를 일으켜 전체 네트워크를 마비시킬 수 있는 위험성을 가지고 있다.
...
볼 수 있다. Ansible 코드에 대한 소개는 여기서는 하지 않는다.
